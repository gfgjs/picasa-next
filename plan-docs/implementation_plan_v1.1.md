# Picasa Next — Implementation Plan v1.1
...
| 编号 | 决策项 | 最终方案 | 架构说明 |
|------|--------|----------|----------|
| Q1 | 哈希算法 | `xxHash3` (xxh3_64) | `cache_key` 字段，**INTEGER (i64)** 原生整数存储，u64 按位重解释为 i64 |
...
| Q13 | 数据库连接模型 | 写 Mutex + 读连接池 | 写连接 `Mutex<Connection>` 保证序列化；读连接 `r2d2` 连接池（4 连接）充分利用 WAL 多读并发 |
| Q14 | 路径存储策略 | 相对路径 + 锚点架构 | 盘符变更只需修改锚点一次 |
| Q15 | 缓存键 | INTEGER (i64)，文件身份绑定 | `xxh3_64("{rel_path}/{file_name}\|{file_mtime}")` → `as i64` 按位重解释，不含 `thumb_size` |
...
| Q27 | XMP 解析 | `quick-xml` 手动解析 | `kamadak-exif` 不支持 XMP，动态照片检测需 XMP 解析 `GCamera:MotionPhoto` / Samsung 标记 |
| Q28 | 窗口状态持久化 | `tauri-plugin-window-state` + `app_config` | 窗口位置/大小/最大化自动持久化；侧边栏宽度、上次目录、排序方式等存 `app_config` |
...
| **连接池** | `r2d2` + `r2d2_sqlite` | latest | 读连接池（4 连接，WAL 多读并发） | Phase 1 |
| **EXIF** | `kamadak-exif` | latest | EXIF 元数据解析 | Phase 1 |
| **XMP** | `quick-xml` | latest | XMP 手动解析（动态照片 GCamera/Samsung 标记检测） | Phase 1 |
...
| `tauri-plugin-window-state` | 窗口位置/大小/最大化状态自动持久化 |
...
│       ├── lib.rs                      # 模块声明
│       ├── error.rs                    # 统一错误类型
│       ├── state.rs                    # AppState（写 Mutex + 读连接池 + 全局状态）
│       │
│       ├── db/
│       │   ├── mod.rs
│       │   ├── connection.rs           # 写连接 + 读连接池 + PRAGMA
│       │   ├── migration.rs            # 版本化 Schema 迁移
...
│   │       ├── ProgressBar.vue
│   │       ├── ThemeToggle.vue
│   │       ├── ErrorBoundary.vue      # 全局错误通知 + IPC 失败处理
...
│   │   ├── useRequestQueue.ts
│   │   ├── useThumbnail.ts
│   │   ├── useMediaDetail.ts          # 组件级 composable（非全局 store）
│   │   ├── useFolderTree.ts
│   │   ├── useTheme.ts
│   │   ├── useSelection.ts
...
│   │   ├── mediaStore.ts
│   │   ├── scanStore.ts
│   │   ├── uiStore.ts                 # 含 UI 状态持久化（侧边栏宽度、排序、上次目录）
...
> [!CAUTION]
> PRAGMA 是连接级设置，写连接和读连接池中的每条连接都需要分别执行，不可仅在迁移脚本中执行一次。读连接池通过 `r2d2::CustomizeConnection` 的 `on_acquire` 回调自动应用。
...
pub struct AppState {
    /// 写连接（扫描入库、更新、删除）— 单连接 Mutex 保证写序列化
    pub db_writer: Mutex<Connection>,
    /// 只读连接池（前端查询）— WAL 下多读并发，无锁竞争
    pub db_read_pool: Pool<SqliteConnectionManager>,
    /// 扫描取消令牌
...
```rust
pub fn init_database(db_path: &Path) -> Result<(Connection, Pool<SqliteConnectionManager>)> {
    // ── 写连接 ──
...
/// 连接池每条连接初始化时自动应用 PRAGMA
struct PragmaCustomizer;
impl r2d2::CustomizeConnection<Connection, r2d2_sqlite::Error> for PragmaCustomizer {
    fn on_acquire(&self, conn: &mut Connection) -> Result<(), r2d2_sqlite::Error> {
        apply_pragmas(conn).map_err(|e| r2d2_sqlite::Error::Other(Box::new(e)))?;
        Ok(())
    }
}
```
...
> [!TIP]
> **为什么不直接用 `Mutex<Connection>` 做读连接？** WAL 模式的核心优势是"多读并发"。单 Mutex 读连接会使所有读查询串行排队，前端快速滚动时 `get_media_layout`、`request_thumbnail`、`get_media_detail` 等查询会争抢同一把锁。读连接池（4 连接）消除了这个瓶颈。
...
INSERT OR IGNORE INTO app_config VALUES ('last_directory_id', '');
INSERT OR IGNORE INTO app_config VALUES ('last_sort_by', 'sort_datetime');
INSERT OR IGNORE INTO app_config VALUES ('last_sort_order', 'desc');
INSERT OR IGNORE INTO app_config VALUES ('sidebar_width', '260');
```
...
CREATE TABLE IF NOT EXISTS media_items (
    ...
    cache_key       INTEGER NOT NULL,                 -- xxh3_64 → as i64 按位重解释
...
> [!NOTE]
> **图片宽高保障**：对于无 EXIF 宽高信息的图片（部分 PNG/WebP/BMP），扫描阶段使用 `image::image_dimensions()` 轻量读取文件头获取（仅解析头部，不解码像素，几乎零开销），确保 `NOT NULL` 约束满足且 Justified Layout 纵横比正确。

> [!NOTE]
> **`cache_key` 类型说明**：`xxh3_64()` 输出 u64，存入 SQLite 时按位重解释为 i64（`as i64`），不损失信息。SQLite INTEGER 底层为有符号 64 位，高位 u64 值会显示为负数，功能不受影响。

> [!NOTE]
> **`updated_at` 更新策略**：SQLite 的 DEFAULT 仅在 INSERT 时生效，UPDATE 操作不会自动触发。Rust 代码中每次 UPDATE 时手动设置 `updated_at = strftime('%s', 'now')`。
...
### 5.6 数据库迁移机制 (`db/migration.rs`)

```rust
/// 版本化迁移：按 schema_version 逐级升级
pub fn run_migrations(conn: &Connection) -> Result<()> {
    ...
    if version < 2 {
        conn.execute_batch("
            -- v2 迁移示例：ALTER TABLE media_items ADD COLUMN new_field TEXT;
        ")?;
        conn.execute(
            "UPDATE app_config SET value = '2' WHERE key = 'schema_version'",
            [],
        )?;
...
> [!TIP]
> 每次 schema 变更添加一个 `if version < N` 块，按顺序执行。开发过程中频繁改 schema 时非常实用——不需要手动删库重建。

### 5.7 Re-scan 策略

当用户移除一个 `scan_root` 后又重新添加相同路径时：

1. 移除操作：CASCADE 删除该 root 下所有 `directories` 和 `media_items`
2. 重新添加：创建新的 `scan_root`（新 ID），触发全量重扫
3. 缩略图缓存不受影响：缓存以 `cache_key`（基于文件路径+mtime）为索引，与数据库 ID 无关
4. 如果文件未发生变化，扫描器通过 mtime 比对可快速跳过已有缩略图
...
| `batch_request_thumbnails` | `{ item_ids }` | `ThumbResult[]` | 批量（**主用接口**） |

> [!IMPORTANT]
> **批量优先策略**：前端请求队列统一攒 16-32 个 item_id 后调用 `batch_request_thumbnails`，单次 IPC 批量处理。减少 IPC round-trip 开销（每次 ~0.5ms），Rust 端用 rayon 并行生成。`request_thumbnail` 仅作为单项补充接口保留。
...
### 7.6 XMP 解析 (`scanner/metadata.rs`)

`kamadak-exif` 不支持 XMP 解析，动态照片检测需要额外的 XMP 处理：

```rust
use quick_xml::events::Event;
use quick_xml::Reader;

/// 检测 Google/Samsung Motion Photo XMP 标记
pub fn detect_motion_photo_xmp(file_path: &Path) -> Result<bool> {
    // 读取文件前 128KB（XMP 通常在文件头部）
    ...
    // 搜索 XMP 起始标记
    if let Some(xmp_start) = find_xmp_packet(data) {
        let xmp_str = std::str::from_utf8(xmp_start).unwrap_or("");
        // 检测 GCamera:MotionPhoto="1" 或 Samsung 标记
        return Ok(
            xmp_str.contains("GCamera:MotionPhoto=\"1\"")
            || xmp_str.contains("MotionPhoto_Data")
        );
    }
    Ok(false)
}
...
> [!TIP]
> XMP 解析仅用于动态照片检测，不需要完整的 XMP 库。`quick-xml` 轻量且纯 Rust，按需解析少量标签即可。
...
│  Step 2: 按类型分发                                              │
│  ├── image  → 8.2.1 图片流水线                                   │
│  ├── video  → 8.2.2 视频流水线 (Phase 2)                        │
│  ├── audio  → 8.2.3 音频流水线 (Phase 2)                        │
│  └── document → 8.2.4 文档流水线 (Phase 2)                      │
│                                                                  │
...
```
┌─────────────────────────────────────────────────────────┐
│  扫描阶段：仅检测标记（XMP/文件尾部），不提取视频数据    │
│  ├── 零额外 I/O，扫描速度不受影响                       │
│  └── 设置 has_embedded_video=1 标记                     │
│                                                         │
│  首次交互时：按需提取并缓存                              │
│  ├── MediaCard hover 1 秒 → 请求 get_companion_video_url│
│  ├── Rust 端检测 cache/motion_videos/ 是否已有缓存       │
│  ├── 未缓存 → 从 JPEG 尾部提取 MP4 → 写入缓存 → 返回   │
│  └── 已缓存 → 直接返回 convertFileSrc URL               │
│                                                         │
│  后续交互：直读缓存，零延迟                              │
└─────────────────────────────────────────────────────────┘
```
...
> [!TIP]
> 首次提取延迟约 50-200ms（取决于嵌入视频大小，通常 2-5MB），用户感知为"短暂加载"后播放，可接受。第二次起直读缓存，零延迟。
...
> [!WARNING]
> **大数据集风险预案**：15 万项一次性加载（4MB JSON 序列化 + IPC 传输 + JS 解析）在当前规模可行，但存在以下风险：
> - JS 端存储 15 万个对象约 50-100MB 内存
> - Justified Layout 全量计算可能阻塞 UI 线程 80-200ms
>
> **缓解策略**（Phase 1 预留接口，按需启用）：
> 1. 当 `total > 50000` 时，Justified Layout 计算放入 **Web Worker**，避免阻塞主线程
> 2. `get_media_layout` 预留 `cursor` 参数，未来可切换为分段加载模式
> 3. Phase 1 先按全量方案实现（1 万张以下无问题），观察实际性能再决定
...
### 10.6 ThumbHash → batch_request → Image.decode() → 缩略图

```
1. thumbhash (28B) → 32x32 DataURL → blur(20px) 占位
2. IntersectionObserver → 请求队列攒 16-32 个 item_id
3. 单次 batch_request_thumbnails IPC → Rust 并行生成
4. 返回批量结果 → 逐个 new Image(url) + img.decode() 预光栅化
5. decode() 完成 → 替换 src → opacity 0→1 (300ms)
快速滚动 → 批量取消离开视口的请求（从队列移除未发送的 batch）
```

> [!TIP]
> **为何批量优先？** 每次 IPC 调用有 ~0.5ms 开销。逐个请求 15 个并发 = 7.5ms/轮 纯开销。攒 32 个一次 batch = 0.5ms 总开销，减少 94% 的 IPC round-trip。
...
```typescript
// uiStore.ts — 启动时从 app_config 读取
async function restoreLastSession() {
  const lastDir = await invoke('get_app_config', { key: 'last_directory_id' })
  const lastSort = await invoke('get_app_config', { key: 'last_sort_by' })
  const lastOrder = await invoke('get_app_config', { key: 'last_sort_order' })
  const sidebarW = await invoke('get_app_config', { key: 'sidebar_width' })
  // 恢复到上次状态
}

// 路由切换 / 排序变更时自动持久化
watch(currentDirectoryId, (id) => {
  invoke('set_app_config', { key: 'last_directory_id', value: String(id ?? '') })
})
```
...
| 配置键 | 说明 | 默认值 |
|--------|------|--------|
| `last_directory_id` | 上次浏览的目录 ID | `''`（全部） |
| `last_sort_by` | 排序字段 | `sort_datetime` |
| `last_sort_order` | 排序方向 | `desc` |
| `sidebar_width` | 侧边栏宽度(px) | `260` |

> [!NOTE]
> 窗口位置/大小/最大化状态由 `tauri-plugin-window-state` 自动处理，无需手动实现。
...
> [!IMPORTANT]
> `useMediaDetail` 是**组件级 composable**，仅在 `MediaDetail.vue` 内部使用，不是全局 Pinia store。其中 `scale`、`translateX`、`translateY` 等高频变化的 UI 状态使用 `ref()` 管理，避免不必要的全局响应式触发。

```typescript
// composables/useMediaDetail.ts — 组件级，每次打开创建新实例
export function useMediaDetail() {
  ...
  // 图片变换状态（高频更新，ref 非 reactive store）
  const scale = ref(1)
  const translateX = ref(0)
  const translateY = ref(0)
  const isOriginalLoaded = ref(false)
  const isLiveVideoPlaying = ref(false)
  ...
  // ... methods: open, close, next, prev, resetTransform, etc.

  return { isOpen, currentItemId, scale, translateX, /* ... */ }
}
```

### 12.8 前端错误边界

全局错误处理策略，Phase 1 即实现：

```typescript
// ErrorBoundary.vue — 全局错误通知组件
// 捕获 IPC 调用失败、缩略图生成错误等
...
interface ErrorNotification {
  ...
  message: string
  dismissible: boolean
  duration?: number  // ms, 0=持久
}
```

| 场景 | 处理方式 |
|------|----------|
| 扫描失败 | Toast 通知 + StatusBar 显示错误状态 |
| 缩略图生成失败 | 卡片显示错误占位图（裂开的图片图标）+ 点击可重试 |
| IPC 调用超时 | Toast "操作超时，请重试" |
| 数据库损坏 | 全屏错误页面 + "重置数据库" 按钮 |
| 不支持的媒体格式 | 详情页显示格式名 + "用外部程序打开" 按钮 |
...
> [!WARNING]
> **不要对 `*` 全局添加 transition**。对所有元素添加 `transition` 会导致 15 万张缩略图容器产生不必要的合成开销，快速滚动时引发 jank。仅对需要主题切换过渡的布局元素添加：
...
- [ ] Cargo.toml Phase 1 依赖（含 r2d2、r2d2_sqlite、quick-xml）
- [ ] CSS 设计系统
- [ ] 目录结构 + `.editorconfig`

#### P1-2 数据库层
- [ ] AppState 写 Mutex + 读连接池 (r2d2, 4 连接)
- [ ] PRAGMA + 建表（`media_items` 含四大类型全部字段预留）
- [ ] migration.rs 版本化迁移机制
- [ ] models / queries / path.rs / format.rs

#### P1-3 文件扫描器
- [ ] walker.rs：遍历 + 格式分类（Phase 1 仅启用图片）
- [ ] metadata.rs：EXIF 解析 + `image::image_dimensions()` 宽高兜底
- [ ] metadata.rs：XMP 解析（quick-xml，动态照片标记检测）
- [ ] live_photo.rs：配对检测 + 嵌入检测（仅标记，不提取视频）
...
#### P1-5 动态照片
- [ ] Motion Photo 嵌入视频延迟提取（首次交互时提取并缓存）
...
- [ ] 缩略图统一走 batch_request_thumbnails（批量优先）
- [ ] Channel + Event 定义
...
- [ ] ThemeToggle
- [ ] ErrorBoundary + Toast（全局错误通知）

#### P1-8 前端逻辑
- [ ] useVirtualScroll / useJustifiedLayout / useRequestQueue
- [ ] useThumbnail（批量优先策略）/ useMediaDetail（组件级 composable）/ useFolderTree / useTheme
- [ ] **filterStore**（含 mediaTypes 筛选 + asMediaFilter getter + 芯片交互逻辑）
- [ ] router + Pinia stores (mediaStore, scanStore, uiStore)
- [ ] **uiStore 状态持久化**（侧边栏宽度、上次目录、排序方式 → app_config）

#### P1 验收标准
- ✅ 1000+ 照片扫描 + Channel 实时进度
- ✅ Justified Layout + ThumbHash → batch_request → Image.decode() → 缩略图
- ✅ 快速滚动 10000+ FPS ≥ 55
- ✅ 大图预览（缩放/拖拽/EXIF）
- ✅ 30MB+ PNG 不 OOM
- ✅ Light/Dark 主题
...
- `cache_key` i64 仅 Rust 内部使用，不传给前端（JS 精度限制）
- `thumbhash` BLOB 前端接收为 `number[]` → `Uint8Array`
- `tauri-plugin-window-state` 需在 `tauri.conf.json` 中注册

### 16.8 数据库迁移

使用版本号递增式迁移（见 5.6 节）。每次 schema 变更添加一个迁移块，启动时自动执行。开发过程中无需手动删库重建。

### 16.9 跨平台注意事项

虽然 Phase 1 仅在 Windows 开发，以下设计决策已为跨平台做好预留：

| 平台 | 注意事项 | 当前应对 |
|------|----------|----------|
| **macOS** | 数据目录在 `~/Library/Application Support/` | Tauri `app_data_dir()` 自动适配 |
| **macOS** | 路径大小写敏感 | 数据库存储原始大小写，不做 toLower |
| **iOS/Android** | WebView 性能较弱 | 虚拟滚动 + ThumbHash 占位 + 批量请求已适配 |
| **iOS/Android** | 文件系统权限受限 | 扫描根目录需通过系统文件选择器授权 |
| **iOS/Android** | 无 FFmpeg sidecar | Phase 4 移动端需评估替代方案（平台原生 API / 编译为静态库） |
| **全平台** | 路径分隔符差异 | 数据库统一 `/`，运行时 `PathBuf` 自动适配 |
| **全平台** | 盘符/挂载点变更 | 相对路径 + 锚点架构 |
...
| Justified Layout 15万项 | ≤ 80ms（超 5 万项移入 Web Worker） |
| 虚拟滚动 FPS | ≥ 55 |
| 内存 15万项 | ≤ 400MB |
| 大图预览 30MB+ PNG | ≤ 3s |
| 动态照片首次提取延迟 | ≤ 200ms |
...
| 音频无封面 | 音符图标占位 |
| r2d2 连接池耗尽 | max_size=4 + busy_timeout 兜底 |
| Schema 迁移失败 | 备份 DB + 回滚日志 |
| 15万项 JS 内存压力 | Web Worker 分流 + 分段加载预留 |
| XMP 解析不完整 | quick-xml 仅解析已知标签，未知标记降级为普通照片 |
