...
use crate::ai::clip::{encode_text, ClipTokenizer, EMBED_DIM, MODEL_NAME};
...
fn ensure_cache(state: &AppState, model_name: &str) -> Result<()> {
    ...
    let mut data: Vec<f16> = Vec::with_capacity(raw.len() * EMBED_DIM);
    for (id, blob) in &raw {
...
pub fn semantic_search(
    ...
    // 2. Make sure the resident cache is loaded (one-time disk read).
    // 2. 确保常驻缓存已加载（一次性磁盘读取）。
    ensure_cache(state, MODEL_NAME)?;
    ...
    let mut scored: Vec<(i64, f32)> = {
        ...
        if cache.is_empty() {
            ...
            conn.execute("DELETE FROM ai_search_results", []).map_err(AppError::from)?;
            return Ok(0);
        }
