...
export interface DirNode {
  ...
  children?: DirNode[]
  absPath?: string
  // Direct files of this directory, lazy-loaded on expand (sidebar tree file list).
  // 本目录的直接文件，展开时懒加载（侧边栏树文件列表）。
  files?: DirFile[]
  filesLoaded?: boolean
}

/** Lightweight media-file row shown under a directory in the sidebar tree. */
/** 侧边栏树中目录下显示的轻量媒体文件行。 */
...
// ── Collection (favorites, 需求7) ─────────────────────────────────────────────
// ── 收藏夹（需求7） ────────────────────────────────────────────────────────────

/** A favorites collection backed by the `albums` table. | 由 albums 表承载的收藏夹。 */
