...
/// Normalise a **scan-root** path for DB storage (§3.8 8A — network drives as scan roots).
/// 规范化**扫描根**路径以便存库（§3.8 8A —— 网络盘当扫描根）。
///
/// Unlike `normalize_db_path`, this preserves a leading UNC prefix (`\\server\share` /
/// `//server/share`) so a network share survives as a root — `normalize_db_path` would strip the
/// `//` and turn the absolute UNC path into a meaningless relative one. Drive-letter (`Z:\…`) and
/// other local paths behave exactly like `normalize_db_path`.
/// 与 `normalize_db_path` 不同，本函数保留前导 UNC 前缀（`\\server\share` / `//server/share`），
/// 使网络共享能作为根存活 —— `normalize_db_path` 会剥掉 `//` 把绝对 UNC 路径变成无意义的相对路径。
/// 盘符（`Z:\…`）等本地路径行为与 `normalize_db_path` 完全一致。
pub fn normalize_root_path(path: &str) -> String {
    let trimmed = path.trim();
    let is_unc = trimmed.starts_with("\\\\") || trimmed.starts_with("//");
    let fwd = trimmed.replace('\\', "/");
    if is_unc {
        // Collapse the leading slash run to exactly two, keep the rest, drop any trailing slash.
        // 把前导连续斜杠折叠为恰好两个，保留其余，去掉尾部斜杠。
        let body = fwd.trim_start_matches('/').trim_end_matches('/');
        format!("//{body}")
    } else {
        fwd.trim_matches('/').to_string()
    }
}
