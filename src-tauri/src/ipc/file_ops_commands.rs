...
fn relocate_cache_files(cache_dir: &Path, renames: &[(i64, i64)]) {
    ...
    for &(old_key, new_key) in renames {
        ...
        for tier in THUMB_TIERS {
            let old_p = thumbs.join(tier.to_string()).join(&old_hex[..2]).join(format!("{old_hex}.webp"));
            if old_p.exists() {
                let new_p = thumbs.join(tier.to_string()).join(&new_hex[..2]).join(format!("{new_hex}.webp"));
                if let Some(parent) = new_p.parent() { let _ = std::fs::create_dir_all(parent); }
                if let Err(e) = std::fs::rename(&old_p, &new_p) {
                    tracing::warn!("thumb relocate failed {:?} -> {:?}: {} | 缩略图重定位失败", old_p, new_p, e);
                }
            }
        }
        ...
        if old_mv.exists() {
            ...
            if let Some(parent) = new_mv.parent() { let _ = std::fs::create_dir_all(parent); }
...
fn copy_dir_recursive(src: &Path, dst: &Path) -> Result<()> {
    ...
    for entry in std::fs::read_dir(src).map_err(|e| AppError::CopyFile(e.to_string()))? {
        ...
        let ft = entry.file_type().map_err(|e| AppError::CopyFile(e.to_string()))?;
        let from = entry.path();
...
async fn move_path_with_fallback(src: &Path, dst: &Path) -> Result<()> {
    ...
    tokio::fs::remove_dir_all(src).await.map_err(|e| AppError::MoveFile(e.to_string()))?;
    Ok(())
}
...
pub async fn move_directory(
    ...
    let (source, target, src_root_path, tgt_root_path) = {
        ...
        if source_dir_id == target_dir_id {
            return Err(AppError::InvalidMove("不能移动到自身 | cannot move into itself".into()));
        }
        if source.parent_id.is_none() {
            return Err(AppError::InvalidMove("不能移动扫描根目录 | cannot move a scan root".into()));
        }
        if source.parent_id == Some(target_dir_id) {
            return Err(AppError::InvalidMove("已位于目标目录中 | already inside the target".into()));
        }
        ...
        if ancestors.contains(&source_dir_id) {
            return Err(AppError::InvalidMove("不能移动到自身的子目录 | cannot move into own descendant".into()));
        }
        ...
        (source, target, src_root_path, tgt_root_path)
    };

    // Guard: refuse while a scan is running on either root (state would corrupt).
    // 守卫：任一根目录正在扫描时拒绝（否则状态会损坏）。
    {
        let tokens = state.scan_tokens.lock().map_err(|e| AppError::System(e.to_string()))?;
        if tokens.contains_key(&source.root_id) || tokens.contains_key(&target.root_id) {
            return Err(AppError::InvalidMove("扫描进行中，请稍后再试 | a scan is running, try again later".into()));
        }
    }
    ...
    let (affected_dirs, affected_media) = {
        ...
        for m in &media {
            let dir_rel = new_rel_by_dir.get(&m.directory_id).cloned().unwrap_or_default();
    ...
    let cache_dir = state.thumb_config.read()
...
pub async fn copy_directory(
    ...
    let (source, target, src_root_path, tgt_root_path) = {
        ...
        if source.parent_id.is_none() {
            return Err(AppError::InvalidMove("不能复制扫描根目录 | cannot copy a scan root".into()));
        }
        if target_dir_id == source_dir_id {
            return Err(AppError::InvalidMove("不能复制到自身 | cannot copy into itself".into()));
        }
        let ancestors = q::get_directory_ancestors(&pool, target_dir_id)?;
        if ancestors.contains(&source_dir_id) {
            return Err(AppError::InvalidMove("不能复制到自身的子目录 | cannot copy into own descendant".into()));
        }
        if q::dir_has_child_named(&pool, target_dir_id, &source.name)? {
            return Err(AppError::DirectoryExists(source.name.clone()));
        }
