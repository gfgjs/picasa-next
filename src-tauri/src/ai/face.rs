...
use crate::ai::clip::l2_normalize;
use crate::ai::engine::{SessionGuard, SessionPool};
use crate::ai::face_profile::{DetectorKind, FaceNorm, FaceProfile};
...
fn letterbox_bgr(decoded: &DecodedImage, size: u32) -> Result<(Array4<f32>, Letterbox)> {
    ...
    if w == 0 || h == 0 {
        return Err(AppError::Internal("empty image for face detect".into()));
    }
    ...
    let src = image::RgbaImage::from_raw(w, h, decoded.pixels.clone())
        .ok_or_else(|| AppError::Internal("RGBA buffer size mismatch".into()))?;
...
fn detect_yunet(
    ...
    let mut guard = detect_pool
        .get()
        .ok_or_else(|| AppError::Internal("face detect pool disconnected".into()))?;
    ...
    let input_name = guard
        ...
        .ok_or_else(|| AppError::Internal("YuNet has no input".into()))?;
    ...
    let input = Tensor::from_array(([1i64, 3, s, s], flat)).map_err(AppError::Ai)?;
    let outputs = guard
...
fn letterbox_rgb_std(decoded: &DecodedImage, size: u32) -> Result<(Array4<f32>, f32)> {
    let (w, h) = (decoded.width, decoded.height);
    if w == 0 || h == 0 {
        return Err(AppError::Internal("empty image for SCRFD detect".into()));
    }
...
fn detect_scrfd(
    ...
    let input_name = guard
        ...
        .first()
        .map(|i| i.name().to_string())
        .ok_or_else(|| AppError::Internal("SCRFD has no input".into()))?;
...
fn align_face(
    ...
    let p = estimate_similarity(&face.landmarks, &profile.align_template)
        .ok_or_else(|| AppError::Internal("similarity transform singular | 相似变换奇异".into()))?;
    let (a, b, tx, ty) = (p[0], p[1], p[2], p[3]);
    // 逆变换（dst→src）：R=[[a,-b],[b,a]] 的逆 = (1/det)[[a,b],[-b,a]]，det=a²+b²。
    let det = a * a + b * b;
    if det.abs() < 1e-12 {
        return Err(AppError::Internal("degenerate transform".into()));
    }
...
fn embed_input_name(guard: &SessionGuard) -> Result<String> {
    guard
        ...
        .first()
        .map(|i| i.name().to_string())
        .ok_or_else(|| AppError::Internal("face embedder has no input".into()))
}
...
fn embed_one(
    ...
    let mut guard = embed_pool
        .get()
        .ok_or_else(|| AppError::Internal("face embed pool disconnected".into()))?;
    let input_name = embed_input_name(&guard)?;
    let input =
        Tensor::from_array(([1i64, 3, size as i64, size as i64], flat)).map_err(AppError::Ai)?;
    let outputs = guard
        .run(vec![(input_name.as_str(), input)])
        .map_err(AppError::Ai)?;
