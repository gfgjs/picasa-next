<template>
  ...
  <Transition name="semantic-panel">
    <div
      ...
      role="region"
      :aria-label="$t('semantic.resultsRegion')"
    >
      <!-- Header -->
      <div class="semantic-panel__header">
        ...
        <div
          ...
          "
        >
          <div class="semantic-panel__progress-bar" :style="{ width: ai.analyzeProgress + '%' }" />
          <span class="semantic-panel__progress-text"> {{ ai.analyzeProgress }}% </span>
        </div>
        ...
        <div class="semantic-panel__controls">
          ...
          <button
            ...
            :aria-label="$t('semantic.rebuildAll')"
          >
            <RefreshCw :size="13" />
          </button>
        </div>
      ...
      <template v-else-if="ai.semanticQuery">
        ...
        <div class="semantic-panel__results-meta">
          ...
          <div class="threshold-slider">
            <label>{{
              $t('semantic.thresholdLabel', { percent: (localThreshold * 100).toFixed(0) })
            }}</label>
            <input
              type="range"
              min="0.1"
...
.semantic-panel__progress-bar {
  ...
  background: linear-gradient(
    90deg,
    var(--color-accent),
    color-mix(in srgb, var(--color-accent) 70%, #a78bfa)
  );
