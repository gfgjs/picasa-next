...
fn main() {
    ...
    let image_model = r"C:\Users\gf\AppData\Roaming\com.picasanext.app\models\vit-b-16.img.fp16.onnx";
    println!("\n[IMAGE MODEL] Loading: {}", image_model);
    println!("[IMAGE MODEL] File size: {:.1} MB (header only, weights in .extra_file)",
        std::fs::metadata(image_model).map(|m| m.len() as f64 / 1024.0 / 1024.0).unwrap_or(0.0));
    ...
    let result = ort::session::Session::builder()
        ...
        .with_intra_threads(4).expect("threads failed")
        .with_optimization_level(ort::session::builder::GraphOptimizationLevel::Disable).expect("opt level failed")
    ...
    match result {
        Ok(session) => {
            ...
            println!("[IMAGE MODEL] Inputs: {:?}", session.inputs().iter().map(|i| i.name()).collect::<Vec<_>>());
        }
        Err(e) => {
            println!("[IMAGE MODEL] ❌ FAILED (Disable) in {:?}: {}", t1.elapsed(), e);
        }
    }
    ...
    let result_l1 = ort::session::Session::builder()
        .expect("builder failed")
        .with_intra_threads(4).expect("threads failed")
        .with_optimization_level(ort::session::builder::GraphOptimizationLevel::Level1).expect("opt level failed")
        .commit_from_file(image_model);
    ...
    match result_l1 {
        ...
        Err(e) => {
            println!("[IMAGE MODEL] ❌ FAILED (Level1) in {:?}: {}", t2.elapsed(), e);
        }
    }
