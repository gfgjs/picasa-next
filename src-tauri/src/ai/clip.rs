...
use crate::ai::profile::{ModelProfile, TokenizerKind};
...
fn run_image_inference(
    ...
    encode_image_batch(session_pool, array, profile)?
        .into_iter()
        .next()
        .ok_or_else(|| AppError::Internal("Empty image inference output | 图像推理输出为空".into()))
}
...
pub fn encode_image_batch(
    ...
    for start in (0..n_total).step_by(chunk) {
        ...
        let tensor = if run_rows > cur {
            ...
            Tensor::from_array((shape, flat_data)).map_err(AppError::Ai)?
        } else {
        ...
        for i in 0..cur {
            ...
            let embedding: Vec<f32> = raw_slice
                .get(off..off + dim)
                .ok_or_else(|| AppError::Internal("Batch output tensor out of bounds".to_string()))?
                .to_vec();
...
pub fn encode_text(
    ...
    let outputs = match text_input_mode {
        TextInputMode::Single(text_input) => {
            let text = Tensor::from_array((shape, ids)).map_err(AppError::Ai)?;
            guard.run(vec![(text_input, text)]).map_err(AppError::Ai)?
        }
        TextInputMode::Bert {
            input_ids,
            attention_mask,
            ...
            let input_ids_tensor = Tensor::from_array((shape, ids)).map_err(AppError::Ai)?;
            let attention_mask_tensor = Tensor::from_array((shape, mask)).map_err(AppError::Ai)?;
            let token_type_ids_tensor = Tensor::from_array((shape, types)).map_err(AppError::Ai)?;
            guard
                .run(vec![
