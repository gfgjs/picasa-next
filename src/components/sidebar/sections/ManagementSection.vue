<template>
  ...
  <AccordionSection v-if="scan.hasScanRoots" id="management" :order="order" :title="$t('sidebar.management')">
      ...
      <div v-for="root in scan.scanRoots" :key="root.id" class="scan-root">
        <div class="scan-root__info">
          ...
          <div class="scan-root__actions">
            ...
            <button class="btn-icon scan-root__remove" :title="$t('sidebar.removeFolder')" @click="removeRoot(root.id)">
              <Trash2 :size="14" />
            </button>
          </div>
        </div>
...
async function removeRoot(id: number) {
  ...
  try {
    const result = await invoke<{ cleared_count: number }>(
      ...
      { id, clearThumbnails: checkboxValue },
    )
...
.scan-root__remove:hover { opacity: 1; }
.scan-root__progress {
  display: flex;
  align-items: center;
  gap: var(--spacing-sm);
